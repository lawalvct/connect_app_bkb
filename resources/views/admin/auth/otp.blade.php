...
<body class="bg-background min-h-screen">
    ...
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            ...
            <!-- OTP Form -->
            <form class="mt-8 space-y-6" method="POST" action="{{ route('admin.auth.otp.verify') }}"
                  x-data="{
                      loading: false,
                      resending: false,
                      countdown: 0,
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex space-x-2 justify-center">
                                <!-- OTP Input Fields -->
                                <div x-data="otpHandler()" class="flex space-x-2">
                                    <template x-for="(digit, index) in digits" :key="index">
                                        <input
                                            ...
                                            class="w-12 h-12 text-center text-lg font-semibold border @error('otp') border-red-500 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                                            autocomplete="off"
                                        >
                                    </template>
                                </div>
                            ...
                            <input type="hidden" name="otp" x-model="getFullOtp()">
    ...
    <script>
        function otpHandler() {
            return {
                ...
                getFullOtp() {
                    return this.digits.map(digit => digit.value).join('');
                },

                updateHiddenInput() {
                    // This will be automatically updated by Alpine's reactivity
                }
            }
        }
        ...
        async function resendOtp() {
            this.resending = true;
            try {
                const response = await fetch('{{ route("admin.auth.otp.resend") }}', {
                    method: 'POST',
                    headers: {
